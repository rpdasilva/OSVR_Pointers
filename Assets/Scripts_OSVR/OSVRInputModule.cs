...
{
    public class OSVRInputModule : PointerInputModule
    {
        [Tooltip("Object which points with Z axis. E.g. VREye from OSVRCameraRig")]
       ...
       // private OSVRRaycaster _activeGraphicRaycaster;
        [NonSerialized]
        public OSVRRaycaster activeGraphicRaycaster;
        [Header("Dragging")]
...
#region PointerEventData pool
        ...
        // Pool for OSVRRayPointerEventData for ray based pointers
        protected Dictionary<int, OSVRRayPointerEventData> m_VRRayPointerData = new Dictionary<int, OSVRRayPointerEventData>();
        ...
        {
            // Get the OSVRRayPointerEventData reference
            OSVRRayPointerEventData leftData;
            ...
            OSVRRaycaster osvrRaycaster = raycast.module as OSVRRaycaster;
            // We're only interested in intersections from OSVRRaycasters
            if (osvrRaycaster) 
            {
                ...
                leftData.position = osvrRaycaster.GetScreenPosition(raycast);
                ...
                if (graphicRect != null)
                {
                    ...
                    OSVRGazePointer.instance.SetPosition(worldPos, normal);
                    // Make sure it's being shown
                    OSVRGazePointer.instance.RequestShow();
                }
            }
            OSVRPhysicsRaycaster physicsRaycaster = raycast.module as OSVRPhysicsRaycaster;
            if (physicsRaycaster)
            {
                leftData.position = physicsRaycaster.GetScreenPos(raycast.worldPosition);
                OSVRGazePointer.instance.RequestShow();
                OSVRGazePointer.instance.SetPosition(raycast.worldPosition, raycast.worldNormal);
            }
            ...
            OSVRRayPointerEventData rightData;
            GetPointerData(kMouseRightId, out rightData, true );
            CopyFromTo(leftData, rightData);
            rightData.button = PointerEventData.InputButton.Right;

            OSVRRayPointerEventData middleData;
        ...
        protected MouseState GetCanvasPointerData()
        {
            ...
            if (activeGraphicRaycaster)
            {
                ...
                OSVRRaycaster osvrRaycaster = raycast.module as OSVRRaycaster;
                if (osvrRaycaster) // raycast may not actually contain a result
                {
                    // The Unity UI system expects event data to have a screen position
                    // so even though this raycast came from a world space ray we must get a screen
                    // space position for the camera attached to this raycaster for compatability
                    Vector2 position = osvrRaycaster.GetScreenPosition(raycast);
        ...
        private bool ShouldStartDrag(PointerEventData pointerEvent)
        {
            ...
            if (pointerEvent as OSVRRayPointerEventData == null)
            {
